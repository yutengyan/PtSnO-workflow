...
核心思想:
    ...
    舍弃异常run后再做集合平均,提高数据质量

算法流程:
    1. 读取每个run的.xvg文件
    2. 提取GMX D值(从注释行 "# D[element] = XXX")
    3. 对同一组成-温度-元素的多个run计算统计量(中位数、标准差)
    4. 检测异常值(3σ原则或IQR原则)
    5. 舍弃异常run后重新做集合平均
    6. 对比改进效果(舍弃前后的R²、D值变化)
...
异常检测方法:
    ...
    方法2: IQR原则(更鲁棒,推荐)
        - 计算四分位数Q1, Q3
        - IQR = Q3 - Q1
        - 舍弃 D < Q1-1.5×IQR 或 D > Q3+1.5×IQR 的run
    
    方法3: 中位数绝对偏差(MAD,最鲁棒)
        - 计算中位数M
        - MAD = median(|D - M|)
        - 舍弃 |D - M| > k×MAD 的run (k=3)

输出:'results里面'
    run_quality_analysis_results/
    ├── run_quality_report.txt          (详细报告)
    ├── large_D_outliers.csv            (被舍弃的run清单)
