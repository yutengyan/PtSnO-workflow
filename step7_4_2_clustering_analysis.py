...
"""
================================================================================
Step 7.4.2: Lindemann指数相态分区分析 (独立版本)
================================================================================
...
数据来源:
========
⚠️ **前置步骤**: 必须先运行 step7_4_multi_system_heat_capacity.py 生成基础数据！

1. 基础数据 (step7_4_all_systems_data.csv):
   来源: step7_4_multi_system_heat_capacity.py输出
   路径: results/step7_4_multi_system/step7_4_all_systems_data.csv
   字段:
   - match_key: 4级路径签名 (用于跨步骤匹配)
   ...
   - phase: step7.4固定阈值分类 (solid/premelting/liquid)
   - run_id: 运行标识 (如r15.gpu0)
   统计: 51个结构, 2370条记录 (应用MSD过滤后)

2. 扩散系数数据 (ensemble_analysis_results.csv, 可选):
   来源: 集合平均MSD重拟合 (Step2输出)
   路径: results/ensemble_analysis_results.csv
   字段: composition, temp_value, element, D_ensemble等
   - D_ensemble: 从集合平均后的MSD曲线重新拟合得到 (MSD = 6Dt)
   - 数据流程: Step1提取GMX的gmx_D用于异常值筛选 → Step2对筛选后的run进行
     集合平均 → 重新线性拟合MSD曲线得到D_ensemble
   - 优势: 集合平均消除单次run随机波动,D值更稳健可靠
   - 匹配策略: 按温度+Pt元素取中位数,再merge到主数据
   - 使用: --use-d-value参数启用
...
# 3. 3D聚类 (加入扩散系数D值)
python step7_4_2_clustering_analysis.py --structure pt6sn8 --n-partitions 3 --use-d-value
...
def load_data(load_d_values=False):
    ...
    # 可选: 合并D值数据
    if load_d_values:
        if not D_VALUE_FILE.exists():
            print(f"  [WARNING] D值数据文件不存在: {D_VALUE_FILE}")
            ...
            # 数据预处理: ensemble数据中包含多个元素,需要聚合
            # 按composition和temperature分组,取Pt元素的D值(主要扩散物种)
            df_D_pt = df_D[df_D['element'] == 'Pt'].copy()
            df_D_pt.rename(columns={'D_ensemble': 'D_value'}, inplace=True)
            
            # 匹配策略:
            # ensemble数据: composition (如 g-1-O1Sn4Pt3) + temp_value
            # step7.4数据: structure (如 pt6sn8) + temp
            # 由于composition命名差异大,采用按温度分组后取中位数的策略
            
            # 按温度计算D值的中位数 (消除composition差异)
            df_D_grouped = df_D_pt.groupby('temp_value').agg({
                'D_value': 'median',  # 使用中位数,更稳健
                'composition': 'count'  # 统计每个温度有多少个composition
            }).reset_index()
            df_D_grouped.rename(columns={
                'temp_value': 'temp',
                'composition': 'n_compositions'
            }, inplace=True)
            
            print(f"      按温度聚合: {len(df_D_grouped)} 个温度点")
            print(f"      每个温度平均 {df_D_grouped['n_compositions'].mean():.1f} 个composition")
            
            # 合并到主数据 (按温度匹配)
            df_merged = df.merge(df_D_grouped[['temp', 'D_value']], on='temp', how='left')
            
            n_matched = df_merged['D_value'].notna().sum()
            n_unique_temps = df_merged[df_merged['D_value'].notna()]['temp'].nunique()
            print(f"      成功匹配D值: {n_matched}/{len(df)} ({n_matched/len(df)*100:.1f}%)")
            print(f"      匹配温度点: {n_unique_temps}")
            
            if n_matched > 0:
                print(f"      D值范围: {df_merged['D_value'].min():.2e} - {df_merged['D_value'].max():.2e} cm^2/s")
                
                # 检查负D值 (物理上不合理,低温拟合误差)
                n_negative = (df_merged['D_value'] < 0).sum()
                if n_negative > 0:
                    print(f"      [WARNING] 检测到 {n_negative} 个负D值 (低温拟合误差),将设为0")
                    df_merged.loc[df_merged['D_value'] < 0, 'D_value'] = 0
                
                df = df_merged
            else:
                print(f"      [WARNING] 无法匹配任何D值,请检查数据格式!")
...
示例用法:
  ...
  python step7_4_2_clustering_analysis.py --structure pt6sn8 --method dbscan --eps 0.3
    ...
    if args.structure.lower() == 'all':
        ...
        structures = [s.strip() for s in args.structure.split(',')]
