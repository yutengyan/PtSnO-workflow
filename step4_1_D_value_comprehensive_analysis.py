...
def plot_D_value_heatmap(df_stats, output_dir):
    """绘制D值热力图 (带R²标注)"""
    print("\n[*] 绘制D值热力图 (带R2标注)...")
    ...
    for series in series_list:
        ...
        # log10标度
        heatmap_data_log = np.log10(heatmap_data.values + 1e-15)
            ...
            for j in range(len(heatmap_data.columns)):
                ...
                if pd.notna(r2_val):
                    if r2_val < 0.1:
                        # R²很低 - 红色标注
                        text_color = 'red'
                        text = f'R²={r2_val:.2f}' if r2_val >= 0.01 else f'R²={r2_val:.3f}'
                        fontweight = 'bold'
                    elif r2_val < 0.5:
                        # R²一般 - 橙色标注
                        text_color = 'orange'
        ...
        ax.set_title(f'扩散系数热力图 - {series}系列 (标注R²值)\nDiffusion Coefficient Heatmap with R² labels',
        ...
        legend_elements = [
            Patch(facecolor='red', label='R² < 0.1 (不可靠)'),
            Patch(facecolor='orange', label='0.1 ≤ R² < 0.5 (一般)'),
            Patch(facecolor='white', label='R² ≥ 0.5 (可靠)')
        ]
...
def plot_R2_heatmap(df_stats, output_dir):
    ...
    for series in series_list:
        ...
        ax.set_xlabel('结构 (Structure)', fontsize=12, fontweight='bold')
        ax.set_ylabel('温度 (Temperature)', fontsize=12, fontweight='bold')
        ax.set_title(f'拟合质量热力图 (R²) - {series}系列\nFit Quality Heatmap (R² values)',
                    fontsize=14, fontweight='bold', pad=15)
        
        cbar = plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
        cbar.set_label('R² (拟合优度)', fontsize=11, fontweight='bold')
...
使用示例:
  ...
  python step4_1_D_value_comprehensive_analysis.py --exclude Pt8Sn0 Pt6Sn0
    ...
    plot_D_value_heatmap(df_stats, OUTPUT_DIR)
