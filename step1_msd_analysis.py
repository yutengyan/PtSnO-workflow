...
输出文件: results/
    ├── large_D_outliers.csv           (异常run清单, run级别)
    ├── all_runs_D_values.csv          (所有runs的D值, run级别, 供聚类分析)
    ├── ensemble_analysis_results.csv  (筛选后的集合平均结果, 组级别)
    ├── ensemble_analysis_filtered.csv (被筛选runs的集合平均, 组级别)
    ├── ensemble_analysis_all.csv      (完整数据, 组级别, 用于--nofilter)
...
def main():
    ...
    # ★ 保存所有runs的D值文件 (run级别, 供聚类分析等使用)
    # 包含每个run的D值、intercept等信息，不区分是否异常
    all_runs_file = Path(OUTPUT_DIR) / 'all_runs_D_values.csv'
    df_runs_export = df_runs[['composition', 'temperature', 'element', 'run_id', 
                               'gmx_D', 'our_D', 'our_r2', 'intercept', 'filepath']].copy()
    # 添加is_outlier标记
    df_runs_export['is_outlier'] = df_runs_export['filepath'].apply(
        lambda x: str(x).lower() in outlier_filepath_set
    )
    df_runs_export.to_csv(all_runs_file, index=False, encoding='utf-8-sig')
    n_outlier_marked = df_runs_export['is_outlier'].sum()
    print(f"  [OK] 所有runs的D值: {all_runs_file}")
    print(f"       ({len(df_runs_export)} runs, 其中 {n_outlier_marked} 异常)")
    ...
    print(f"    - all_runs_D_values.csv: {total_runs} 条记录 (run级别D值,供聚类分析使用)")
